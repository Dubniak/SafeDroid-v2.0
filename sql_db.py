...
class SafeDroidDB:
    ...
    def initConnection(self):
        try:
            self.db = MySQLdb.connect(self.config.host(), self.config.username(), self.config.password(), self.config.schema())
        except OperationalError:
            self.buildDB()
            self.db = MySQLdb.connect(self.config.host(), self.config.username(), self.config.password(), self.config.schema())
            self.log.info('Create schema %s @ %s') % (self.config.schema() ,self.config.host())
        return
    
    def buildDB(self):
        db = MySQLdb.connect(self.config.host(), self.config.username(), self.config.password())
        cursor = db.cursor()
        q = " CREATE DATABASE `%s`; " % self.config.schema()
        try:
            cursor.execute(q)
        except:
            self.log.critical('Failed to create %s schema' % self.config.schema())
            pass
    ...
    def dropTable(self, name):
        d = MySQLdb.connect(self.config.host(), self.config.username(), self.config.password(), self.config.schema())
        c = d.cursor()
        q = "DROP TABLE "+ name
        try:
            c.execute(q)
    ...
    def updateToTable(self, tname, id = None, md5=None, mal_cnt = None, ben_cnt = None, isMalicious = None, ratio = None ):
        ...
        if ('API' in tname in table_list) :
            if mal_cnt != None :
                upd = """ UPDATE `%s` SET `MalOcc`= %d WHERE `ApiId` = '%d' """ % (tname , mal_cnt, id)
                self.executeQuery(upd,False)
    
            if ben_cnt != None:
                upd = """ UPDATE `%s` SET `BenOcc`= %d WHERE `ApiId` = '%d' """ % (tname , ben_cnt, id)
                self.executeQuery(upd,False)
                
            if ratio != None:
                upd = """ UPDATE `%s` SET `Ratio`= %f WHERE `ApiId` = '%d' """ % (tname , ratio, id)
                self.executeQuery(upd,False)
                
            if isMalicious != None:
                upd = """ UPDATE `%s` SET `IsMalicious`= %d WHERE `AppId` = '%d' """ % (tname , isMalicious, id)
                self.executeQuery(upd,False)
                
        elif ('PERMISSION' in tname in table_list):
            if mal_cnt != None :
                upd = """ UPDATE `%s` SET `MalOcc`= %d WHERE `PrmId` = '%d' """ % (tname , mal_cnt, id)
                self.executeQuery(upd,False)
                
            if ben_cnt != None:
                upd = """ UPDATE `%s` SET `BenOcc`= %d WHERE `PrmId` = '%d' """ % (tname , ben_cnt, id)
                self.executeQuery(upd,False)
            if ratio != None:
                upd = """ UPDATE `%s` SET `Ratio`= %f WHERE `PrmId` = '%d' """ % (tname , ratio, id)
                self.executeQuery(upd,False)
            if isMalicious != None:
                upd = """ UPDATE `%s` SET `IsMalicious`= %d WHERE `PrmId` = '%d' """ % (tname , isMalicious, id)
                self.executeQuery(upd,False)
    ...
    def setRatio(self, tname):
        ...
        for row in range(1,numberofrows) : 
            r = """SELECT `MalOcc`,`BenOcc` FROM `%s` WHERE `%s` = '%d' """ % (tname,idname, row)
    ...
    def exists(self, tname, md5, name):
        ...
        '''
        if 'APPLICATIONS' in tname:
            q = """SELECT * FROM `APPLICATIONS` WHERE `Md5` = '%s' AND `Name`='%s'; """ % (md5 , name)
