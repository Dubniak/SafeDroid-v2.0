...
class Tune:
	...
	def fineTuneClassifiers(self):
		for model in self.models:
			self.Learning_Curve_Plot(self.fv[self.maxi][self.maxj]['vector'],self.sd.target[self.maxj], model, display=self.config.display_plots(), filename = self.config.plot_to_file())
			self.Validation_Curve(self.fv[self.maxi][self.maxj]['vector'],self.sd.target[self.maxj], model, display=self.config.display_plots(), filename = self.config.plot_to_file())
			if self.config.display_plots() or self.config.plot_to_file():
				self.ROC(self.fv[self.maxi][self.maxj]['vector'],self.sd.target[self.maxj],model)
		return
					
	def tuneClassifiers(self):
		results = self.parameter_estimation(self.fv[self.maxi][self.maxj]['vector'],self.sd.target[self.maxj],None)
		opt_clsf , tuning, acc = self.getBestClassifier(results)
		best_tune = self.formTuneDictionary()
		for k,v in tuning.iteritems():
			best_tune[k] = v
		self.models.append(Model(opt_clsf, best_tune, acc=acc, dss = self.fv[self.maxi][self.maxj]['vector'].shape[0]))
		
		#second classifier
		alter_clsf = self.Cross_Validation_Plot(self.fv[self.maxi][self.maxj]['vector'],self.sd.target[self.maxj],self.fv[self.maxi][self.maxj]['info'])
		alter_clsf_value = max(alter_clsf.iteritems(), key=operator.itemgetter(1))[0] # fetch name
		
		if alter_clsf_value is opt_clsf:
			print 'Reached on agreement for Optimum Classifier'
		else : 
			result  = self.parameter_estimation(self.fv[self.maxi][self.maxj]['vector'],self.sd.target[self.maxj],alter_clsf_value)
			best_tune = self.formTuneDictionary()
			for k,v in result[alter_clsf_value][-1]['tune'].iteritems():
				best_tune[k] = v
			self.models.append(Model(alter_clsf_value, best_tune, acc = result[alter_clsf_value][2][result[alter_clsf_value][0]['score']], dss = self.fv[self.maxi][self.maxj]['vector'].shape[0]))
		return
	...
	def Validation_Curve(self,X,y,model,display=False,filename=None):
		...
		try:
			if classifier == 'KNeighbor':
				param_range = np.array(filter(lambda x: x% 2 != 0, list(range(4,30))))
				param_name = "n_neighbors"
				xtitle = 'neighbors'
				train_scores, test_scores = validation_curve(KNeighborsClassifier(weights=tune['weights'],algorithm=tune['algorithm']), X, y, param_name=param_name, param_range=param_range,cv=self.config.cv(), scoring=scoring, n_jobs=multiprocessing.cpu_count())
			elif classifier == 'ADA':
				param_range = np.array(filter(lambda x: x% 2 != 0, list(range(4,30))))
				param_name = 'n_estimators'
				xtitle = 'estimators'
				train_scores, test_scores = validation_curve(AdaBoostClassifier(algorithm = tune['algorithm']), X, y, param_name=param_name, param_range=param_range,cv=self.config.cv(), scoring=scoring, n_jobs=multiprocessing.cpu_count())
			elif classifier == 'SVM':
				param_range = np.logspace(-6, -1, 5)
				param_name = 'gamma'
				xtitle = "$\gamma$"
				train_scores, test_scores = validation_curve(SVC(kernel=tune['kernel'],C=tune['C']), X, y, param_name=param_name, param_range=param_range,cv=self.config.cv(), scoring=scoring, n_jobs=multiprocessing.cpu_count())
			elif classifier == 'DTree':
				param_range = np.array(filter(lambda x: x% 5 == 0, list(range(4,50))))
				param_name = 'max_depth'
				xtitle = 'depth'
				train_scores, test_scores = validation_curve(DecisionTreeClassifier(class_weight=tune['weights']), X, y, param_name=param_name, param_range=param_range,cv=self.config.cv(), scoring=scoring, n_jobs=multiprocessing.cpu_count())
			elif classifier == 'RForest':
				param_range = np.array(filter(lambda x: x% 2 != 0, list(range(4, 30))))
				param_name = 'n_estimators'
				xtitle = 'estimators'
				train_scores, test_scores = validation_curve(RandomForestClassifier(min_samples_split = tune['min_samples_split'],oob_score=tune['oob_score'], class_weight=tune['class_weight']), X, y, param_name=param_name, param_range=param_range,cv=self.config.cv(), scoring=scoring, n_jobs=multiprocessing.cpu_count())
			
		except ValueError,err:
			self.log.error("Cannot validate estimation graph for %s.\nReason:" %classifier)
			self.log.error(err)
			return	
		...
		if display or filename:
			self.plot_validation_curve(classifier, xtitle, "Score", param_range, train_scores_mean, train_scores_std, test_scores_std, test_scores_mean)
			if filename:
	...
	def Learning_Curve_Plot(self,X,y,model,display=False,filename=None): #1
		...
		cv = ShuffleSplit(n_splits=self.config.cv(), test_size=self.config.test_size(), random_state=0)
		
		try:
			if classifier is 'KNeighbor':
				train_sizes, train_scores, test_scores = learning_curve(KNeighborsClassifier(n_neighbors=tune['n_neighbors'], weights=tune['weights'], algorithm = tune['algorithm']), X, y, cv=cv, n_jobs=multiprocessing.cpu_count(), train_sizes=np.linspace(.1, 1.0, 5))
			elif classifier is 'SVM':
				train_sizes, train_scores, test_scores = learning_curve(SVC(kernel=tune['kernel'],C=tune['C'],gamma=tune['gamma']), X, y, cv=cv, n_jobs=multiprocessing.cpu_count(), train_sizes=np.linspace(.1, 1.0, 5))
			elif classifier is 'DTree':
				train_sizes, train_scores, test_scores = learning_curve(DecisionTreeClassifier(min_weight_fraction_leaf = tune['min_weight_fraction_leaf'], class_weight = tune['class_weight'], min_samples_split = tune['min_samples_split']), X, y, cv=cv, n_jobs=multiprocessing.cpu_count(), train_sizes=np.linspace(.1, 1.0, 5))
			elif classifier is 'RForest':
				train_sizes, train_scores, test_scores = learning_curve(RandomForestClassifier(n_estimators= tune['n_estimators'],min_samples_split=tune['min_samples_split'],oob_score=tune['oob_score'], class_weight = tune['class_weight']), X, y, cv=cv, n_jobs=multiprocessing.cpu_count(), train_sizes=np.linspace(.1, 1.0, 5))
			elif classifier is 'MLP':
				train_sizes, train_scores, test_scores = learning_curve(MLPClassifier(solver = tune['solver'], activation = tune['activation']), X, y, cv=cv, n_jobs=multiprocessing.cpu_count(), train_sizes=np.linspace(.1, 1.0, 5))
			elif classifier is 'ADA':
				train_sizes, train_scores, test_scores = learning_curve(AdaBoostClassifier(algorithm=tune['algorithm'], n_estimators=tune['n_estimators']), X, y, cv=cv, n_jobs=multiprocessing.cpu_count(), train_sizes=np.linspace(.1, 1.0, 5))
		except ValueError , err:
			self.log.error("Cannot estimate learning curve for %s.\nReason:" %classifier)
			self.log.error(err)
			return	
		...
		model.setConvergence(self.isConverging(train_scores_mean, test_scores_mean))
		model.setPassLimit(np.max(train_scores_mean) >= 0.9)
		
		if display or filename:
			self.plot_learning_curve(_title[classifier],train_sizes,train_scores,test_scores,train_scores_mean,train_scores_std,test_scores_mean,test_scores_std,ylim=(0.7, 1.01))
			if filename:
	...
	def show_confusion_matrix(self,C,class_labels=['0','1']):	    
	    ...
	    ax.text(2,2,
			  'Accuracy: %.2f \nSpecificity: %.2f \nPrecision: %.2f'%((tp+tn+0.)/N , (tn+0.)/NN, (tp+0.)/(tp+fp)),
			  va='center',
			  ha='center',
			  bbox=dict(fc='w',boxstyle='round,pad=1'))
	...
	def Cross_Validation_Plot(self,X,Y, info, display=False, filename = None): #1
		...
		for name, model in models:
			try:
				kfold = model_selection.KFold(n_splits=self.config.cv(), random_state=seed)
				cv_results = model_selection.cross_val_score(model, X, Y, cv=kfold, scoring=scoring, n_jobs=multiprocessing.cpu_count())
		...
		if self.config.display_plots() or self.config.plot_to_file() :
			...
			title1 = "\nData Set Size:" + str(info['overall_size']) + " , Malicious Apps:" + str(info['mal_size']) + " (" + "{:.2f}".format(info['mal_ratio']*100)+ "%)"
	...
	def ROC(self,X,y,model):
		...
		if clsf is 'KNeighbor':
			classifier = KNeighborsClassifier(n_neighbors=tune['n_neighbors'], weights=tune['weights'], algorithm = tune['algorithm'])
		elif clsf is 'SVM':
			classifier = SVC(kernel=tune['kernel'],C=tune['C'],gamma=tune['gamma'], random_state = random_state, probability = True)
		elif clsf is 'DTree':
			classifier = DecisionTreeClassifier(min_weight_fraction_leaf = tune['min_weight_fraction_leaf'], class_weight = tune['class_weight'], min_samples_split = tune['min_samples_split'], random_state = random_state)
		elif clsf is 'RForest':
			classifier = RandomForestClassifier(n_estimators= tune['n_estimators'],min_samples_split=tune['min_samples_split'],oob_score=tune['oob_score'], class_weight = tune['class_weight'], random_state = random_state)
		elif clsf is 'MLP':
			classifier = MLPClassifier(solver = tune['solver'], activation = tune['activation'], random_state = random_state)
		elif clsf is 'ADA':
			classifier = AdaBoostClassifier(algorithm=tune['algorithm'], n_estimators=tune['n_estimators'], random_state = random_state)
	...
	def parameter_estimation(self,X,y,classifier):
		X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=self.config.test_size(), random_state=0)
		...
		if classifier is None:
			...
			for class_name, classifier in _classifiers.iteritems():
				...
				for score in scores:
					partial = []
					print("# %s: Tuning hyper-parameters for %s" % (class_name, score))
					try:
						clf = GridSearchCV(classifier,_tuned_parameters[class_name], cv = self.config.cv(), scoring='%s' %score, n_jobs= multiprocessing.cpu_count() )
						clf.fit(X_train, y_train)
					except ValueError, err:
						print 'Exception'
						print err
						self.log.error("Cannot estimate parameters for %s.\nReason:"%classifier)
						self.log.error(err)
						break
					...
					indi_scores = {qq[0]:float(qq[17]) , qq[1]:float(qq[18]) , qq[2]:float(qq[19])}
					partial.append({'individual':indi_scores})
					partial.append({score:round(np.max(clf.cv_results_['mean_test_score']),4)})
			...
			for score in scores:
				partial = []
				print (" Tunning parameters for alternative classifier.......%s" %classifier)
				try:
					clf = GridSearchCV(_classifiers[classifier],_tuned_parameters[classifier], cv = self.config.cv(), scoring='%s' %score, n_jobs= multiprocessing.cpu_count() )
					clf.fit(X_train, y_train)
				except ValueError, err:
					self.log.error("Cannot estimate parameters for %s.\nReason:"%classifier)
					self.log.error(err)
