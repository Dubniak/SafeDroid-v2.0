...
def perform_analysis_marios(d,x,no_connection):
        data = []
        data.append(
                                    { 'analysis_results' :
                                            [
        ...
        data.append( # i can get it by md5sum on the APK/VIR file
				{ 'apk_file' :
					[
...
def generate_API_List(x):
    ...
    '''
    #method_analysis = [k.get_tags() for k in x.get_methods()]
    pkg_list = [item for sublist in [k.get_tags() for k in x.get_methods()] for item in sublist]
    return set(pkg_list)
