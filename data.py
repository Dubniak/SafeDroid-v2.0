...
class Data:
    def __init__(self,dir_name): 
        ...
        _files= ['api.csv', 'applications.csv','apptoapi.csv','apptoprm.csv','permission.csv']
        ...
        self.sizes = []
        self.app = self.init_app(os.path.join(os.getcwd(),dir_name,_files[1]), _columnNames[1])
        self.api = API(os.path.join(os.getcwd(),dir_name,_files[0]), _columnNames[0],[ self.app.getMaliciousSize() , self.app.getBenignSize()])
        self.apptoapi = self.init_apptoapi(os.path.join(os.getcwd(),dir_name,_files[2]), _columnNames[2])
        ...
        self.data_sets = self.formDataSets() # list of dicts, dict keys = ['data_sets'],['info']
    ...
    def formDataSets(self):
		...
		for reduction in self.config.reduce_size():
			...
			for malicious_perc in self.config.malicious_size():
				...
				mask_malicious = np.random.choice([False,True],len(self.app.malicious),p=[1-float(malicious_size)/len(self.app.malicious), float(malicious_size)/len(self.app.malicious)])
				malicious_set = self.app.malicious.iloc[mask_malicious]
				
				#create redused benign set
				mask_benign = np.random.choice([False,True], len(self.app.benign), p=[1-float(benign_size)/len(self.app.benign),float(benign_size)/len(self.app.benign)])
				benign_set = self.app.benign.iloc[mask_benign]
				
				#concatenate two sets and shuffle entries
				dic['data_set'] = pd.concat([malicious_set,benign_set], axis=0).sample(frac=1)
				dic['info'] = self.addDataSetSizeInfo(len(malicious_set),float(len(malicious_set))/(len(malicious_set)+len(benign_set)),len(benign_set),1-float(len(malicious_set))/(len(malicious_set)+len(benign_set)),len(malicious_set)+len(benign_set),reduction)
    ...
    def apisOfAppListDF(self):
		...
		for ds in self.data_sets:
			...
			for i in range(0, length):
				ID = matrix.index[i]
				application.append(self.api._matrix.loc[self.apptoapi._matrix[self.apptoapi._matrix.appid==ID].apiid])
